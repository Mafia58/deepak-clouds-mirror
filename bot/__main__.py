...
'''
    ...
    buttons.buildbutton("Support Group", "https://t.me/DFX_LEECH")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    LOGGER.info('UID: {} - UN: {} - MSG: {}'.format(update.message.chat.id, update.message.chat.username, update.message.text))
    uptime = get_readable_time((time.time() - botStartTime))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        if update.message.chat.type == "private" :
            sendMessage(f"Hey Besike kolla. Marila na itim. dnno ne @akalankanime Keriyek tmi. 🙂\nSince: <code>{uptime}</code>", context.bot, update)
        else :
            update.effective_message.reply_photo(IMAGE_URL, start_string, parse_mode=ParseMode.MARKDOWN, reply_markup=reply_markup)
    else :
        sendMessage(f"Hey Besike kolla. Marila na itim. dnno ne @akalankanime Keriyek tmi. ubata botwa use krnna ba. sorry pocca", context.bot, update)
...
def main():
    ...
    LOGGER.info("නැගිට්ටා ඉතිම් උබලා මට ඉන්න දෙන්නෙ නෑ නෙ!")
